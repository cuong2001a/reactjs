...
const UserAdminEdit = () => {
    ...
    const readUser = async () => {
        try {
            const { data } = await UserAPI.findOne(id)
            console.log(data)
            setUser(data)
        } catch (error) {
            console.log(error)
                            ...
                            <div>
                                <form onSubmit={handleSubmit(editUser)}>
                                    <div className=" mb-3 input-group gap-5">
                                        <div className="w-25">
                                            <label for="exampleFormControlInput1" className="form-label">Tên người dùng</label>
                                            <input type="text" className="form-control"
                                                defaultValue={user ? user.name : ""} {...register('name', { required: true })} />
                                            {errors.name && <span >Không được để trống tên người dùng</span>}
                                        </div>
                                        <div className="w-25">
                                            <label for="exampleFormControlInput1" className="form-label">Email</label>
                                            <input type="email" className="form-control"
                                                defaultValue={user ? user.email : ""} {...register('email', { required: true })} />
                                            {errors.email && <span >Không được để trống gmail</span>}
                                        </div>
                                        <div className="w-25">
                                            <label for="exampleFormControlInput1" className="form-label">Phân quyền</label>
